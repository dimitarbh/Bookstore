...
document.addEventListener("DOMContentLoaded", function() {
    function isWithinLast30Or31Days(epochTimestamp) {
        console.log("Epoch Timestamp:", epochTimestamp);
        const millisecondsInDay = 86400000; 
        const currentDate = new Date();
        const currentTimestamp = currentDate.getTime();
        const thirtyOr31DaysAgoTimestamp = currentTimestamp - (currentDate.getDate() === 31 ? 30 : 31) * millisecondsInDay;

        return epochTimestamp >= thirtyOr31DaysAgoTimestamp;
    }
    ...
    {{#each books}}
        <li class="book">
            ...
            {{/if}}
            <p class=" hide">Top Selling: {{#if topSelling}}Yes{{else}}No{{/if}}</p>
            {{#if (isWithinLast30Or31Days this.createdAt)}}
                <p class="created-at additional-info hide">Created At: {{formatDate this.createdAt}}</p>
            {{/if}}
            <div class="bottom-content">
                <button class="toggle-btn">Read more</button>
            </div>
...
`;
    ...
    Handlebars.registerHelper("isWithinLast30Or31Days", function(createdAt) {
        return isWithinLast30Or31Days(new Date(createdAt));
    });
    ...
    .then(data => {
        ...
        const last30Or31DaysBooks = data.filter(book => isWithinLast30Or31Days(new Date(book.createdAt)));
        const booksDiv = document.getElementById('books');
        
        booksDiv.innerHTML = template({ books: last30Or31DaysBooks });
        console.log("Rendered Books:", last30Or31DaysBooks);
        booksDiv.addEventListener('click', toggleAdditionalInfo);
    })
    .catch(error => {
        console.error('There was a problem fetching the books:', error);
